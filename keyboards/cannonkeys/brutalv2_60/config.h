...
/* 2 bits are represented in the base address */
/* the other 8 bits are transferred over wire */
/* 10 bits total to address the 1024 bytes */

#define EXTERNAL_EEPROM_I2C_ADDRESS(loc) \
        (EXTERNAL_EEPROM_I2C_BASE_ADDRESS | ((((loc) >> 8) & 0x03) << 1))
